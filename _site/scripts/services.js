const searchInput=document.querySelector("#services-search-input");let services=[];function fetchData(t){return fetch(t).then(e=>{if(e.ok)return e.json();throw new Error(`Failed to fetch data from ${t}: ${e.status} `+e.statusText)}).catch(e=>{throw console.error(e),new Error(`Fetching data from ${t} failed. Please check your network connection
